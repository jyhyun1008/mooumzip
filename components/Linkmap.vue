    ...
    if (process.server){
        ...
        for (let i=0; i<myDataJson.map.items.length; i++){
            ...
            if (itemPlaced.position.z == 0) {
                if (isAvatar) {
                    // floor1 += `<img src="${itemUrl}" class="map-avatar" style="top: calc(236px + 16 * ${itemPlaced.position.x}px + 16 * ${itemPlaced.position.y}px); left: calc(-32px + 32 * ${itemPlaced.position.x}px - 32 * ${itemPlaced.position.y}px); z-index: ${20+itemPlaced.position.x+itemPlaced.position.y};" />`
                } else {
                    if (itemPlaced.isFlipped == false) {
                    floor1 += `<img src="${itemUrl}" style="top: calc(268px + 16 * ${itemPlaced.position.x}px + 16 * ${itemPlaced.position.y}px); left: calc(-32px + 32 * ${itemPlaced.position.x}px - 32 * ${itemPlaced.position.y}px); z-index: ${20+itemPlaced.position.x+itemPlaced.position.y};" />`
                } else {
                    floor1 += `<img src="${itemUrl}" style="top: calc(268px + 16 * ${itemPlaced.position.x}px + 16 * ${itemPlaced.position.y}px); left: calc(-32px + 32 * ${itemPlaced.position.x}px - 32 * ${itemPlaced.position.y}px); transform: scaleX(-1); -webkit-transform: scaleX(-1); z-index: ${20+itemPlaced.position.x+itemPlaced.position.y};" />`
                }
            }
        }
    }
